...
def _random_walk_on_track_circ_graph(
    ...
    """
    Circular random walk on a 1D track using shortest wrap-around distance:

        d_circ = min(distance, track_length - distance)

    Parameters
    ----------
    place_bin_centers : np.ndarray, shape (n_position_bins,)
        The position (in cm or arbitrary units) of each bin.
    movement_mean : float
    movement_var : float
    ...
    Returns
    -------
    state_transition : np.ndarray of shape (n,n)
        Transition probabilities BEFORE row-normalization.
    """

    n = place_bin_centers.size
